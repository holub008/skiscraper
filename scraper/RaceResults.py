...
class RaceInfo:
    ...
    def __init__(self, season, division, d, u, n):
        ...
        self.season = unicode(season)
        self.division = unicode(division)
        self.date = unicode(d)
        self.url = unicode(u)
        self.name = unicode(n)

    @staticmethod
    def create_empty(s, d):
        """
        :param s: the season of the race, as a year (str)
        :param d: race division (str)
        """
        return RaceInfo(s, d, None, None, None)
    ...
    def serialize(self, cursor, result_type):
        ...
        raw_sql = "INSERT INTO %s" % (RACE_DB, ) \
                  + " (rname, rdate, ryear, rurl, result_type, division) VALUES (%s, %s, %s, %s, %s, %s)"

        cursor.execute(raw_sql, (self.name, self.date, self.season, self.url, result_type, self.division))
    ...
    def __eq__(self, other):
        """
            minimal uniquely identifying set of criteria
        """
